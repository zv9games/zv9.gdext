...
use crate::obj::{EngineBitfield, Gd, GodotClass};
...
impl<'c, R: ParamTuple + IntoDynamicSend> TypedSignal<'c, R> {
    /// Creates a fallible future for this signal.
    ///
    /// The future will resolve the next time the signal is emitted.
...
impl<'c, R: ParamTuple + IntoDynamicSend> IntoFuture for &TypedSignal<'c, R> {
    type Output = R;
